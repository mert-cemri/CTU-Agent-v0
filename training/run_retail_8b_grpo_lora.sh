...
# GRPO Training with LoRA on Retail Domain - 8B Model
# This script trains Qwen3-8B with LoRA adapters for memory-efficient training
...
EXPORT_DIR="$CTU_ROOT/exports/tau_bench_retail_8b_lora_${TIMESTAMP}"
if [ ! -d "$CKPT_DIR" ]; then
...
echo "========================================="
echo "Starting 8B GRPO Training with LoRA on Retail Domain"
echo "========================================="
...
echo "WandB Project: tau_bench_retail_grpo_8b_lora"
echo ""
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.run_name="retail_8b_grpo_lora_r${LORA_RANK}_$(date +%Y%m%d_%H%M%S)" \

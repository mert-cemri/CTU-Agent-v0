...
# GRPO Training on Retail Domain - 8B Model (No Taxonomy)
# This script trains Qwen3-8B on retail domain with pure task-based rewards only
...
# Enable VLLM settings (optimized for 8B model)
...
echo "========================================="
echo "Starting 8B GRPO Training on Retail Domain"
echo "========================================="
echo "Model: $POLICY_MODEL"
echo "Domain: Retail only"
echo "Taxonomy Feedback: DISABLED (Pure task rewards)"
echo "WandB Project: tau_bench_retail_grpo_8b_pure"
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.project_name="tau_bench_retail_grpo_8b_pure" \
  trainer.run_name="retail_8b_grpo_pure_$(date +%Y%m%d_%H%M%S)" \

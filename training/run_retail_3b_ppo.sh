...
# PPO Training on Retail Domain - 3B Model
# This script trains Qwen2.5-3B-Instruct on retail domain using PPO with GAE
...
MODEL_NAME_SANITIZED=$(echo $POLICY_MODEL | tr '/' '_')_retail_ppo_v1
...
EXPORT_DIR="$CTU_ROOT/exports/tau_bench_retail_ppo_${RUN_TIMESTAMP}"
if [ ! -d "$CKPT_DIR" ]; then
...
# Explicitly disable taxonomy feedback for vanilla PPO
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.run_name="retail_3b_ppo_$(date +%Y%m%d_%H%M%S)" \
...
echo "Algorithm: PPO with GAE (gamma=$gamma, lambd=$lambd)"
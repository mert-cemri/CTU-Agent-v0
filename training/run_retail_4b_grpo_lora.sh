...
# GRPO Training with LoRA on Retail Domain - 4B Model
# This script trains Qwen3-4B with LoRA adapters for memory-efficient training

# Configuration for 4B model with LoRA
NUM_GPUS=8
...
MODEL_NAME_SANITIZED=$(echo $POLICY_MODEL | tr '/' '_')_retail_grpo_lora_v0
...
EXPORT_DIR="$CTU_ROOT/exports/tau_bench_retail_4b_lora_${TIMESTAMP}"
if [ ! -d "$CKPT_DIR" ]; then
...
echo "========================================="
echo "Starting 4B GRPO Training with LoRA on Retail Domain"
echo "========================================="
...
echo "WandB Project: tau_bench_retail_grpo_4b_lora"
echo ""
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.project_name="tau_bench_retail_grpo_4b_lora" \
  trainer.run_name="retail_4b_grpo_lora_r${LORA_RANK}_$(date +%Y%m%d_%H%M%S)" \
...
echo "Training approach: Parameter-efficient fine-tuning with LoRA adapters"
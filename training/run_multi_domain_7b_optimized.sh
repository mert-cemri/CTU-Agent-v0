...
# Optimized Multi-domain training for 7B model (OOM prevention)
...
MODEL_NAME_SANITIZED=$(echo $POLICY_MODEL | tr '/' '_')_v6-multi-domain-optimized

# Configuration for 7B model to avoid OOM
NUM_GPUS=8
...
# Enable VLLM to use longer context lengths
...
echo "========================================="
echo "Starting 7B Model Training (OOM-Optimized)"
echo "========================================="
echo "Model: $POLICY_MODEL"
echo "GPUs: $NUM_GPUS"
echo "Tensor Parallel: $TENSOR_PARALLEL_SIZE"
echo "Max Context: 16384"
echo ""

# Run with optimized memory settings for 7B model
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.project_name="tau_bench_rl_7b" \
  trainer.run_name="tau_bench_7b_optimized_$(date +%Y%m%d_%H%M%S)" \

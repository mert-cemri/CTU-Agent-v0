...
# GRPO + Taxonomy Feedback Training on multi Domain - 3B Model
# This script trains Qwen2.5-3B-Instruct on multi domain with LLM Judge feedback
...
# POLICY_MODEL="mcemri/qwen2.5_3b_alldata_sft_v0"  # e.g., "/root/ckpts/your_sft_model" or "your-hf-username/your-sft-model"
...
# MODEL_NAME_SANITIZED=$(echo $POLICY_MODEL | tr '/' '_')_multi_grpo_taxonomy_after_sft_v3
MODEL_NAME_SANITIZED=$(echo $POLICY_MODEL | tr '/' '_')_multi_grpo_taxonomy_v14 #v13 vs 13-gpu0 are good
...
EXPORT_DIR="$CTU_ROOT/exports/tau_bench_multi_taxonomy_${RUN_TIMESTAMP}"
if [ ! -d "$CKPT_DIR" ]; then
...
echo "========================================="
echo "Starting 3B GRPO + Taxonomy Training on multi Domain"
echo "========================================="
echo "Model: $POLICY_MODEL"
echo "Domain: multi only"
echo "Taxonomy Feedback: ENABLED (alpha=$TAXONOMY_ALPHA)"
echo "WandB Project: tau_bench_multi_grpo_with_taxonomy_feedback"
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.run_name="multi_3b_grpo_taxonomy_alpha${TAXONOMY_ALPHA}_$(date +%Y%m%d_%H%M%S)" \

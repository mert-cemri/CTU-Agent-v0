...
# GRPO + Taxonomy Feedback with LoRA Training on Retail Domain - 4B Model
# This script trains Qwen3-4B with LoRA adapters and LLM Judge feedback
...
EXPORT_DIR="$CTU_ROOT/exports/tau_bench_retail_4b_lora_taxonomy_${TIMESTAMP}"
if [ ! -d "$CKPT_DIR" ]; then
...
echo "========================================="
echo "Starting 4B GRPO + Taxonomy with LoRA on Retail Domain"
echo "========================================="
...
echo "WandB Project: tau_bench_retail_grpo_4b_lora_with_taxonomy"
echo ""
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.run_name="retail_4b_grpo_lora_taxonomy_r${LORA_RANK}_alpha${TAXONOMY_ALPHA}_$(date +%Y%m%d_%H%M%S)" \

...
# GRPO + Taxonomy Feedback Training on Retail Domain - 7B Model
# This script trains Qwen2.5-7B-Instruct on retail domain with LLM Judge feedback
...
echo "========================================="
echo "Starting 7B GRPO + Taxonomy Training on Retail Domain"
echo "========================================="
echo "Model: $POLICY_MODEL"
echo "Domain: Retail only"
echo "Taxonomy Feedback: ENABLED (alpha=$TAXONOMY_ALPHA)"
echo "WandB Project: tau_bench_retail_grpo_7b_with_taxonomy_feedback"
...
HYDRA_FULL_ERROR=1 python main_tau_bench.py \
  ...
  trainer.run_name="retail_7b_grpo_taxonomy_alpha${TAXONOMY_ALPHA}_$(date +%Y%m%d_%H%M%S)" \

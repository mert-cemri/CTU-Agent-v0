...
"""
    ...
    def chat_completion_request_openai(self, prompt: str) -> str:
        ...
        try:
            ...
            # Convert potentially non-serializable API errors to simple RuntimeError for Ray compatibility
            error_message = str(e)
            if "InternalServerError" in str(type(e)) or ("missing" in error_message and "positional arguments" in error_message):
                raise RuntimeError(f"OpenAI API call failed: {error_message}") from None
            else:
                raise
